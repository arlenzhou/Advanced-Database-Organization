 ...
 *
 * Description:
 *		Initialize the Record Manager with a given entry to the buffer manager
 *
 * Parameters:
 *		void *mgmtData: entry of the buffer manager
 *
 * Return:
 *		RC: return code
 *
 ...
 *
 * Parameters:
 * 		RM_TableData *rel: table manager entry
 ...
 *
 * Description:
 *		Close the passed in table
 *
 * Parameters:
 * 		RM_TableData *rel: the table manager we about to close
 *
 * Return:
 *		RC: return code
 *
 ...
 *
 * Description:
 *		Get the total records number stored in the table
 *
 * Parameters:
 * 		RM_TableData *rel: table manager entry
 *
 * Return:
 *		int: total records number
 *
 * Author:
 *		Chengnan Zhao <czhao18@hawk.iit.edu>
 *
 ...
 *
 * Description:
 *		Update the total records number to a user entered value
 *
 * Parameters:
 * 		RM_TableData *rel: table manager entry
 ...
 *
 * Description:
 *		Get the total number of pages stored in the table
 *
 * Parameters:
 * 		RM_TableData *rel: table manager entry
 *
 * Return:
 *		int: the total page number
 *
 * Author:
 *		Chengnan Zhao <czhao18@hawk.iit.edu>
 *
 ...
 *
 * Description:
 *		Update the total number of pages to a user entered value
 *
 * Parameters:
 * 		RM_TableData *rel: table manager entry
 ...
 *
 * Description:
 *		Get the max number of records that can be stored in one page
 *
 * Parameters:
 * 		RM_TableData *rel: table manager entry
 *
 * Return:
 *		int: the max records number
 *
 * Author:
 *		Chengnan Zhao <czhao18@hawk.iit.edu>
 *
 ...
 *
 * Return:
 *		int: the total tuples number in the table
 *
 * Author:
 *		Chengnan Zhao <czhao18@hawk.iit.edu>
 *
 ...
 *
 * Description:
 *		Check the passed in RID is valid
 *
 * Parameters:
 * 		RM_TableData *rel: table manager entry
 ...
 *
 * Description:
 *		Get the reserved page number that records the passed in page
 *
 * Parameters:
 * 		PageNumber pageNum: the page number of the page we about to check
 *
 * Return:
 ...
 *
 * Description:
 *		Check the passed in page has empty slots
 *
 * Parameters:
 * 		RM_TableData *rel: table manager entry
 ...
 *
 * Description:
 *		Get the value stored at a certain bit (TRUE/FALSE)
 *
 * Parameters:
 ...
 *
 * Description:
 *		Set the value at a certain bit (TRUE/FALSE)
 *
 * Parameters:
 *		char *data: data entry
 *		int bitOffset: bit offset
 *		bool value: the value to be set with(TRUE/FALSE)
 *
 * Return:
 *		RC: return code
 *
...
RC setBit(char *data, int bitOffset, bool value) {
	...
	if (divResult.rem == 0) {
		...
		byteOffset = divResult.quot;
		bitPosition = sizeof(char) - divResult.rem;
	}
 ...
 *
 * Description:
 *		Pin the reserved page and initialize it
 *
 * Parameters:
 *		RM_TableData *rel: table manager entry
 *		PageNumber reservedPageNum: page number of the reserved page
 *
 * Return:
 *		RC: return code
 *
 ...
 *
 * Description:
 *		Update the passed in reserved page
 *
 * Parameters:
 * 		RM_TableData *rel: table manager entry
 *		PageNumber reservedPageNum: the page number of the reserved page we are working on
 *		PageNumber PageNum: the page number of the page we about to change
 *		bool value: the bit value we about to update with (TRUE/FALSE)
 *
 * Return:
 *		RC: return code
 *
...
RC updateReservedPage(RM_TableData *rel, PageNumber reservedPageNum,
	...
	if (value == FALSE && pageNum <= firstFreePageNum) {
		...
		while (bitOffset < maxBitOffset && getBit(page->data, bitOffset) != 1) {
			bitOffset++;
		}
 ...
 *
 * Description:
 *		Get the page number of the first page with free slots
 *
 * Parameters:
 *		RM_TableData *rel: table manager entry
 *		PageNumber reservedPageNum: the page number of the reserved page
 *
 * Return:
 *		PageNumber: number of the page we found
 *
 * Author:
 *		Jie Zhou <jzhou49@hawk.iit.edu>
 *
 ...
 *
 * Description:
 *		Update the page number of the first page on a reserved page
 *
 * Parameters:
 *		RM_TableData *rel: table manager entry
 *		PageNumber reservedPageNum: the page number of the reserved page
 *		int value: the user entered page number of the first free page
 *
 * Return:
 *		RC: return code
 *
 ...
 *
 * Description:
 *		Get the page number of next reserved page on the current reserved page
 *
 * Parameters:
 *		RM_TableData *rel: table manager entry
 *		PageNumber reservedPageNum: the current reserved page number
 *
 * Return:
 *		PageNumber: the next reserved page number
 *
 * Author:
 *		Jie Zhou <jzhou49@hawk.iit.edu>
 *
 ...
 *
 * Description:
 *		Update the page number of next reserved page on the current reserved page
 *
 * Parameters:
 *		RM_TableData *rel: table manager entry
 *		PageNumber reservedPageNum: the page number of the current reserved page
 *		int value: the user entered next reserved page number
 *
 * Return:
 *		RC: return code
 *
 ...
 *
 * Description:
 *		Get the page number of previous reserved page on the current reserved page
 *
 * Parameters:
 *		RM_TableData *rel: table manager entry
 *		PageNumber reservedPageNum: the page number of the current reserved page
 *
 * Return:
 *		PageNumber: the page number of the previous reserved page
 *
 * Author:
 *		Jie Zhou <jzhou49@hawk.iit.edu>
 *
 ...
 *
 * Description:
 *		Update the page number of previous reserved page on the current reserved page
 *
 * Parameters:
 *		RM_TableData *rel: table manager entry
 *		PageNumber reservedPageNum: the page number of the current reserved page
 *		int value: the user entered previous reserved page number
 *
 * Return:
 *		RC: return code
 *
 ...
 *
 * Description:
 *		Search for the page number of the first page with free slots
 *
 * Parameters:
 *		RM_TableData *rel: table manager entry
 *
 * Return:
 *		PageNumber: the page number we found
 *
 * Author:
 *		Jie Zhou <jzhou49@hawk.iit.edu>
 *
 ...
 *
 * Description:
 *		Check if a certain page is initialized
 *
 * Parameters:
 *		RM_TableData *rel: table manager entry
 *		PageNumber pageNum: the page we want ot check
 *
 * Return:
 *		bool: is the page initialized or not
 *
 * Author:
 *		Xin Su <xsu11@hawk.iit.edu>
 *
 ...
 *
 * Description:
 *		Update a certain page's initialization status
 *
 * Parameters:
 *		RM_TableData *rel: taable manager entry
 *		PageNumber pageNum: the page number of the page we want to update
 *		bool value: the user entered initialization status
 *
 * Return:
 *		RC: return code
 *
 ...
 *
 * Description:
 *		Get the number of records stored in a certain page
 *
 * Parameters:
 *		RM_TableData *rel: table manager entry
 ...
 *
 * Return:
 *		int: the number of records stored in the page
 *
 * Author:
 *		Xin Su <xsu11@hawk.iit.edu>
 *
 ...
 *
 * Description:
 *		Update the number of records stored in a certain page
 *
 * Parameters:
 *		RM_TableData *rel: table manager entry
 *		PageNumber pageNum: the page number of the page we want to update
 *		int value: the user entered records number
 *
 * Return:
 *		RC: return code
 *
 ...
 *
 * Description:
 *		Check if a certain slot is occupied
 *
 * Parameters:
 *		RM_TableData *rel: table manager entry
 *		PageNumber pageNum: the page number of the page we want to check
 *		int slotId: the slot number of the slot we want to check
 *
 * Return:
 *		bool: a slot is occupied or not
 *
 * Author:
 *		Xin Su <xsu11@hawk.iit.edu>
 *
 ...
 *
 * Description:
 *		Update a certain slot's occupancy status
 *
 * Parameters:
 *		RM_TableData *rel: table manager entry
 *		PageNumber pageNum: the page number of the page we want to update
 *		int slotId: the slot number of the slot we want to update
 *		bool value: the occupied status user entered
 *
 * Return:
 *		RC: return code
 *
 ...
 *
 * Description:
 *		Check if the passed in record has a valid length
 *
 * Parameters:
 *		RM_TableData *rel: table manager entry
 *		char *recordData: entry of a record's data
 *
 * Return:
 *		RC: return code
 *
 ...
 *
 * Description:
 *		Initialize a new record page
 *
 * Parameters:
 *		RM_TableData *rel: table manager entry
 *		PageNumber pageNum: the page number of the page we about to initialize
 *
 * Return:
 *		RC: return code
 *
 ...
 *
 * Description:
 *		Get the slot number of the first free slot on a certain page
 *
 * Parameters:
 *		RM_TableData *rel: table manager entry
 *		PageNumber pageNum: the page number of the page we want to check
 *
 * Return:
 *		int: the slot number of the slot we found
 *
 * Author:
 *		Xin Su <xsu11@hawk.iit.edu>
 *
 ...
 * Parameters:
 *		RM_TableData *rel: table manager entry
 *		Record *record: the pointer to the record we about to insert
 *
 * Return:
 *		RC: return code
 *
 ...
 *
 * Parameters:
 * 		RM_TableData *rel : table: table manager entry
 ...
 * Parameters:
 *		RM_TableData *rel: table manager entry
 *		Record *record: the passed in record
 *
 * Return:
 *		RC: return code
 *
 ...
 * Parameters:
 *		RM_TableData * rel: table manager entry
 *		RID id: RID of the record we about to get
 *		Record * record: a pointer to the record we about to get
 *
 *
 * Return:
 *		RC: return code
 ...
 *
 * Description:
 *		Initialize a new scan manager with given table manager and given expression
 *
 * Parameters:
 *		RM_TableData * rel: table manager entry
 ...
 *
 * Description:
 *		Get the next eligible record in the table
 *
 * Parameters:
 ...
 *
 * Description:
 *		Get the size of the each record of given schema
 *
 * Parameters:
 ...
 *
 * Parameters:
 *		Schema *schema: the schema we about to parse
 *
 * Return:
 ...
 *
 * Description:
 *		Create a new schema with given values
 *
 * Parameters:
 ...
 *
 * Parameters:
 *		Schema *schema: the schema we about to free
 *
 * Return:
 *		RC: return code
 *
 ...
 *
 * Description:
 *		Create an empty record of given schema
 *
 * Parameters:
...
RC createRecord(Record **record, Schema *schema) {
	...
	char *rData = (char *) calloc(RECORD_LENGTH, sizeof(char));
 ...
 *
 * Parameters:
 * 		Record *record: the record we about to unset
 *
 * Return:
 *		RC: return  code
 *
 ...
 *
 * Description:
 *		Get the value of the attrNumth attribute for given record of given schema
 ...
 *
 * Description:
 *		Update the value of the attrNumth attribute for given record to given value
 *
 * Parameters:
 * 		Record *record: a record about to be updated
 * 		Schema *schema: the schema of the given record
